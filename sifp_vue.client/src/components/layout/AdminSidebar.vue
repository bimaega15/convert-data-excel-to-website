...
// Grup worksheet dibangun otomatis dari manifest backend, sehingga jumlah & isi
// menu selalu mengikuti worksheet pada workbook yang terakhir diimport.
// computed, bukan snapshot, agar menu ikut berubah setelah import ulang.
const worksheetGroups = computed(() =>
...
<template>
  <aside class="sidebar" :class="{ 'sidebar--open': open, 'sidebar--collapsed': collapsed }">
    ...
    <div class="sidebar__footer">
      <p class="sidebar__source" :title="meta.sourceFile">{{ meta.sourceFile || 'Belum ada import' }}</p>
      <p class="sidebar__version">Import: {{ meta.generatedAt?.slice(0, 10) || '-' }}</p>
    </div>
  </aside>
</template>

...
// Isi sheet diambil per slug saat halaman dibuka, jadi workbook dengan puluhan
// sheet pun tidak perlu dikirim sekaligus ke browser.
let controller = null
...
async function load(slug) {
  ...
  try {
    data.value = await api.get(`/api/worksheets/${encodeURIComponent(slug)}`, { signal })
  } catch (err) {
    if (err.name === 'AbortError') return
    // 404 berarti slug-nya memang tidak ada di workbook terakhir — itu kondisi
    // normal (mis. link lama), bukan gangguan server.
...
<template>
  <div class="master-page">
    ...
    <div v-if="notFound" class="panel sv__state">
      ...
      <p>
        Worksheet <strong>{{ route.params.slug }}</strong> tidak ada pada workbook yang
        terakhir diimport.
      </p>
    </div>

    <div v-else-if="error" class="panel sv__state sv__state--error">
      <DashIcon name="warning" :size="22" />
      <div>
        <p><strong>Gagal memuat worksheet.</strong></p>
        <p class="sv__state-detail">{{ error.message }}</p>
      </div>
      <button type="button" class="sv__retry" @click="load(route.params.slug)">Coba lagi</button>
    </div>

...
const { rows, loading, error, reload } = useApiRows((signal) =>
  fetchAllPages('/api/master/drift-conditions', { signal })
)

const { deleting, deleteError, onDelete } = useDeleteRows(
  (keys) => api.post('/api/master/drift-conditions/delete', { ids: keys }),
  reload
)
...
async function handleAdd(formData) {
  ...
  submitError.value = ''
  try {
    await api.post('/api/master/drift-conditions/create', formData)
    showModal.value = false
    reload()
  } catch (err) {
    submitError.value = err.message || 'Gagal menambah drift condition baru.'
  } finally {
  ...
  <div class="master-page">
    ...
    <DataState :loading="loading" :error="error" :empty="!rows.length" @retry="reload">
      <DataTable
        ...
        selectable
        can-add
        add-label="Tambah Drift Condition"
        row-key="key"
        :deleting="deleting"
    ...
    <AddRowModal
      :show="showModal"
      title="Tambah Drift Condition Baru"
      :columns="columns"
